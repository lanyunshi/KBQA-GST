...
Then, download and save the pre-processed datasets ([ComplexWebQuestion](https://www.tau-nlp.org/compwebq) and [WebQuestionsSP](https://www.microsoft.com/en-us/download/details.aspx?id=52763)) from [here]() under the `data/` directory. The data contains questions, answers, results of topic entity linking, unit linking and the candidate paths based on the entity linking, unit linking respectively.

## Download Pre-trained models
Create a folder `saved-model` under the current directory. Download the pre-trained models from [here]()
...
To generate the final prediction, run `python code/GenerateFinalPredictions.py`.\
