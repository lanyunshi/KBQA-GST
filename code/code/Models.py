...
class Model(nn.Module):
    ...
    def EE_forward(self, t, q, p, r, y, an, anlen, orig_l, l, elmos, dic):
        ...
        _logit, _prob = self.execution_engine(_t, _q, _p, _r, _y, _an, _anlen)
        #print(self.execution_engine.emb_init.weight[10, :10])
    ...
    def train(self, train, dev, test, dic, outfile):
        ...
        for ep in range(args.max_epoches):
            ...
            for i in range(N):
                ...
                if args.model_type == 'EE':
                    t, q, p, r, y, an, anlen, orig_l, l, elmos = obtain_topic_entity_candidate_path_features(
                                                    train, train_shuffle_batch[i], [dic, dic2], is_EE = True)
                    ...
                    t, q, els, elo, cate, alia, orig_l, el, others = obtain_topic_entity_candidate_path_features(
                                                    train, train_shuffle_batch[i], [dic, dic2])
                    q_ = q
            ...
            if args.only_eval:
                ...
                shuffle_batch = create_batches(dev.len(), args.batch)#[:1]
            ...
            for i in range(len(shuffle_batch)):
                if args.model_type == 'EE':
                    batch_data = obtain_topic_entity_candidate_path_features(
                                                        dev, shuffle_batch[i], [dic, dic2], is_EE = True)
                else:
                    batch_data = obtain_topic_entity_candidate_path_features(
                                                        dev, shuffle_batch[i], [dic, dic2])
                ...
                for b in range(k):
                    ...
                    cand_path_num = 0 if b >= an.shape[0] else np.sum(an[b]> 0) if args.model_type in ['TG'] else np.sum(np.max(an[b], 1)> 0)
                    if i % args.display == 0:
                ...
                if dev_eval > min_dev_eval:
                    ...
                    for i in range(N):
                        if args.model_type == 'EE':
                            time1 = time.time()
                            batch_data = obtain_topic_entity_candidate_path_features(
                                                                test, shuffle_batch[i], [dic, dic2], is_EE = True)
                        else:
                            batch_data = obtain_topic_entity_candidate_path_features(
                                                                test, shuffle_batch[i], [dic, dic2])
                    ...
                    if False: #args.model_type in ['TG+EE'] and ('WBQ' in args.test_q):
