...
class Dataset(data.Dataset):
    def __init__(self, path, dic = None, limit = 1000, topic_limit = 450, is_train = False, use_units = True):
        ...
        if is_train:
            ...
            self.to_entities, self.coincident = self.read_topic_entities(os.path.join(path, 'topic_entities.json'),
                                                        units_file, dic, topic_limit = topic_limit) # topicentity_all_types_copy
        if topic_limit:
            self.cand_paths, self.cand_paths_score, self.cand_paths_an, self.cand_path_cont = self.read_candidate_paths(candidate_file,
                                                        dic, limit = limit, entities = self.to_entities) #candidate_paths_SMART
        else:
            self.cand_paths, self.cand_paths_score, self.cand_paths_an, self.cand_path_cont = self.read_candidate_paths(candidate_file,
                                                        dic, limit = limit) # candidate_paths_EnrichSMART
