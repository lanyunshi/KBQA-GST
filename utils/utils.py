...
def generate_predicate(cand_path, is_CHARLSTM = 1):
	rels = []
	for c in cand_path:
		if len(c.split('\t')) == 2:
			x0, r1 = c.split('\t')
			...
			exit()
		if is_CHARLSTM == 1:
			rel = ['_'.join(r.split('.')[-2:]).split('_') for r in rs] #
			rel = ' '.join(sum(rel, []))
		elif is_CHARLSTM == 2:
			rel = [r.split('.')[-1] for r in rs] #
			rel = ' '.join(rel)
		rels += [rel]
	return rels
...
def obtain_topic_entity_candidate_path_features(data, batches, dic, is_EE = False, use_elmo = False, is_CHARLSTM = False):
	dic, dic2 = dic
	t = np.zeros((len(batches), 100), dtype = np.int32)
	q = np.zeros((len(batches), 100), dtype = np.int32)
	if use_elmo:
		elmo_t = torch.FloatTensor(np.zeros((len(batches), 100, 3072), dtype = np.float32))
		elmo_q = torch.FloatTensor(np.zeros((len(batches), 100, 3072), dtype = np.float32))
	if is_CHARLSTM == 1:
		q_char = np.zeros((len(batches), 100, 20), dtype = np.int32)
		alia_char = np.zeros((len(batches), 500, 100, 20), dtype = np.int32)
		max_char = 1
	if is_EE:
		...
		if use_elmo:
			elmo_p = torch.FloatTensor(np.zeros((len(batches), 2000, 100, 3072), dtype = np.float32))
			elmo_r = torch.FloatTensor(np.zeros((len(batches), 2000, 100, 3072), dtype = np.float32))
	else:
	...
	for i, b in enumerate(batches):
		...
		if use_elmo:
			input = [sentence.split(' ')] + [template.split(' ')]
		if is_CHARLSTM == 1:
			for word_idx, word in enumerate(sentence.split()):
				char_idx = words2index(word, dic)[:20]
				q_char[i, word_idx, :len(char_idx)] = char_idx
				if len(char_idx) > max_char:
					max_char = len(char_idx)
		...
		if is_EE:
			...
			if use_elmo:
				input = use_elmo.batch_to_embeddings(input)[0][:, :, :, :].view(len(input), -1, 3072)
				num, max_len, _ = input.size()
				elmo_q[i, :max_len, :] = input[0, :, :].cpu()
				elmo_t[i, :max_len, :] = input[1, :, :].cpu()
				if int((num-2)/2) > 0:
					elmo_p[i, :int((num-2)/2), :max_len, :] = input[2:2+int((num-2)/2), :, :].cpu()
					elmo_r[i, :int((num-2)/2), :max_len, :] = input[2+int((num-2)/2):num, :, :].cpu()
			...
			for m_idx, mid in enumerate(to_entity):
				...
				try:
					...
					exit()
				if is_CHARLSTM == 1:
					for word_idx, word in enumerate(ali.split()):
						char_idx = words2index(word, dic)[:20]
						alia_char[i, m_idx, word_idx, :len(char_idx)] = char_idx
						if len(char_idx) > max_char:
							max_char = len(char_idx)
	...
	if is_EE:
		...
		if use_elmo:
			elmo_t = elmo_t[:, :max_tlen, :]
			elmo_q = elmo_q[:, :max_qlen, :]
			elmo_p = elmo_p[:, :max_cand, :max_plen, :]
			elmo_r = elmo_r[:, :max_cand, :max_rlen, :]
			return t, q, p, r, y, an, anlen, orig_l, l, (elmo_t, elmo_q, elmo_p, elmo_r)
		else:
		...
		if is_CHARLSTM == 1:
			alia_char = alia_char[:, :max_el, :max_alen, :max_char]
			q_char = q_char[:, :max_qlen, :max_char]
			return t, (q, q_char), els, elo, cate, (alia, alia_char), orig_l, el, (topic_entities, answers, to_entities, to_entities2)
		else:
